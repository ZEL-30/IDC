/*
 *  程序名: migratetable_oracle.cpp, 本程序出数据中心的公共功能模块, 用于迁移表中的数据
 *  作者: ZEL
 */
#include "_public.h"
...
connection     conn;          // 用于执行查询、插入和删除的SQL语句的数据库连接 
...
bool _migratetable() {
    ...
    // 从数据字典中获取表中全部的字段名
    if (!TABCOLS.allcols(&conn, starg.dsttname)) {
