        ...
        while (true) { 
            ...
            if (conn.m_state == 0) {
                if (conn.connecttodb(starg.connstr, starg.charset) != 0) {
                    logfile.Write("connect database(%s) falied.\n", starg.connstr);
                    return false;
                }
            ...
            // 调用处理XML文件的子函数
            logfile.WriteEx("处理文件%s ... ", Dir.m_FullFileName);
            ...
            if (iret == 1 || iret == 2 || iret == 5) {
                ...
                if (iret == 5) logfile.WriteEx("failed, 待入库的表(%s)字段太多.\n", stxmltotable.tname);
            ...
            if (stmtins.m_cda.rc == 1) {
                ...
                if (stxmltotable.uptbz == 1) {
                    if (stmtupt.execute() != 0) {
                        ...
                        // 数据库连接已失效, 无法继续, 只能返回
                        // 1053-在操作过程中服务器关闭, 2013-查询过程中丢失了与Oracle服务器的连接
...
void crtSQL() { 
    ...
    for (int i = 0; i < TABCOLS.m_vallcols.size(); i++) {
        // upttime字段不处理
        if (strcmp(TABCOLS.m_vallcols[i].colname, "upttime") == 0) continue;

        // 拼接strinsertp1
        strcat(strinsertp1, TABCOLS.m_vallcols[i].colname);
        strcat(strinsertp1, ","); 

        // 拼接strinsetp2, 注意需要区分keyid字段, date字段和非date字段
        if (strcmp(TABCOLS.m_vallcols[i].colname, "keyid") == 0) {
            ...
            if (strcmp(TABCOLS.m_vallcols[i].datatype, "date") != 0) 
                ...
                SPRINTF(strtemp, sizeof(strtemp), "to_date(:%d, 'yyyymmddhh24miss')", colseq);
    ...
    // 生成修改表的SQL语句
    // update T_ZHOBTMIND1 set t=:1,p=:2,u=:3,wd=:4,wf=:5,r=:6,vis=:7,upttime=sysdate,mint=:8,minttime=str_to_date(:9,'%Y%m%d%H%i%s') where obtid=:10 and ddatetime=str_to_date(:11,'%Y%m%d%H%i%s')
    ...
    for (int i = 0; i < TABCOLS.m_vallcols.size(); i++) {
        ...
        if (strcmp(TABCOLS.m_vallcols[i].datatype, "date") != 0)
            ...
            SPRINTF(strtemp, sizeof(strtemp), "%s = to_date(:%d, 'yyyymmddhh24miss')", TABCOLS.m_vallcols[i].colname, uptseq);
    ...
    for (int i = 0; i < TABCOLS.m_vallcols.size(); i++) {
        ...
        if (strcmp(TABCOLS.m_vallcols[i].datatype, "date") != 0)
            ...
            SPRINTF(strtemp, sizeof(strtemp), " and %s = to_date(:%d, 'yyyymmddhh24miss')", TABCOLS.m_vallcols[i].colname, uptseq);
