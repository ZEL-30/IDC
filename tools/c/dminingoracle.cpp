...
#define MAXPARAMS 256    // 结果集字段的最大值
...
struct st_arg {
    ...
    char pname[51];         // 进程名, 建议用 “dminingoracle后缀” 的方式
} starg;
...
void _help() {
    ...
    printf("Sample:/project/tools/bin/procctl 3600 /project/tools/bin/dminingoracle /log/idc/dminingoracle_ZHOBTCODE1.log \"<connstr>qxidc/19981110@snorcl11g_197</connstr><charset>Simplified Chinese_China.AL32UTF8</charset><selectsql>select obtid,cityname,provname,lat,lon,height from T_ZHOBTCODE1</selectsql><fieldstr>obtid,cityname,provname,lat,lon,height</fieldstr><fieldlen>10,30,30,10,10,10</fieldlen><bfilename>ZHOBTCODE1</bfilename><efilename>HYCZ</efilename><outpath>/idcdata/dmindata1</outpath><timeout>30</timeout><pname>dminingoracle_ZHOBTCODE1</pname>\"\n\n");
    printf("       /project/tools/bin/procctl   30 /project/tools/bin/dminingoracle /log/idc/dminingoracle_ZHOBTMIND1.log \"<connstr>qxidc/19981110@snorcl11g_197</connstr><charset>Simplified Chinese_China.AL32UTF8</charset><selectsql>select obtid,to_char(ddatetime,'yyyymmddhh24miss'),t,p,u,wd,wf,r,vis,keyid from T_ZHOBTMIND1 where keyid>:1 and ddatetime>sysdate-0.04</selectsql><fieldstr>obtid,ddatetime,t,p,u,wd,wf,r,vis,keyid</fieldstr><fieldlen>10,19,8,8,8,8,8,8,8,15</fieldlen><bfilename>ZHOBTMIND1</bfilename><efilename>HYCZ</efilename><outpath>/idcdata/dmindata1</outpath><starttime></starttime><incfield>keyid</incfield><incfilename>/idcdata/dmining/dminingoracle_ZHOBTMIND1_HYCZ.list</incfilename><timeout>30</timeout><pname>dminingoracle_ZHOBTMIND1_HYCZ</pname><maxcount>1000</maxcount><connstr1>qxidc/19981110@snorcl11g_197</connstr1>\"\n\n");
...
bool _xmltoarg(const char *strxmlbuffer) {
    ...
    for (int i = 0; i < CmdStr.CmdCount(); i++) {
        ...
        if (ifieldlen[i] > imaxfieldlen) 
            imaxfieldlen = ifieldlen[i];   // 字段的长度最大不能超过imaxfieldlen
    }
